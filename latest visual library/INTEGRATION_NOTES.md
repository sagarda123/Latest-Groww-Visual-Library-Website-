...
Vercel **Root Directory = `latest visual library`** (the site lives in this
subfolder). Serverless auth functions live in `api/` (relative to that root).
Set the `AUTH_USERNAME` / `AUTH_SALT` / `AUTH_PASSWORD_HASH` / `AUTH_HMAC_SECRET`
